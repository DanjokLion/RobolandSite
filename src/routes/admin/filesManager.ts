...
router.get( '/', async ( req: express.Request, res: express.Response, next: express.NextFunction ) => {
  try {
    const folders: any = []
    const files: any = []
    fs.readdirSync(process.cwd() + '/public/uploads/images/').forEach( ( file: any ) => {
...
router.post( '/', async ( req: express.Request, res: express.Response, next: express.NextFunction ) => {
  try {
    ...
    fs.readdirSync(process.cwd() + '/public/uploads/images'+req.body.path ).forEach( ( file: any ) => {
      const re = /([a-zA-Z0-9\s_\\.\-\(\):])+(.jpg|.png|.webp)$/i
      if ( re.test( file ) == true ) {
        arrFiles.push(file)
      } else {
...
router.post( '/upload', async ( req: express.Request, res: express.Response, next: express.NextFunction ) => {
  try {
    ...
    if (req.files) {
      img.mv( process.cwd() + '/public/uploads/images' + ( req.body.path || '' ) + '/' + img.name, function(err: any) {
        if ( err ) {
...
router.post( '/rename', async ( req: express.Request, res: express.Response, next: express.NextFunction ) => {
  try {
    ...
    fs.renameSync( process.cwd() + '/public/uploads/images'+req.body.old, process.cwd() + '/public/uploads/images'+req.body.new )

    fs.readdirSync( process.cwd() + '/public/uploads/images'+parrentDir ).forEach( ( file: any ) => {
      const re = /([a-zA-Z0-9\s_\\.\-\(\):])+(.jpg|.png|.webp)$/i
      if ( re.test( file ) == true ) {
        arrFiles.push(file)
      } else {
...
router.post( '/delete', async ( req: express.Request, res: express.Response, next: express.NextFunction ) => {
  try {
    ...
    fs.unlinkSync( process.cwd() + '/public/uploads/images'+req.body.file )
